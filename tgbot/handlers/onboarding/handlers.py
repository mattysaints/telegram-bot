...
async def command_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(text=screen.text, reply_markup=render_start_menu(screen), parse_mode=ParseMode.HTML)


async def start_menu_action(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await query.answer()
    await query.message.reply_text(menu_action_message(action_key))
